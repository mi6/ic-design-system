...
export const createReactHTML = (ext: string) => `<html lang="en">
      <head>
        <meta charset="UTF-8" />
...
export const createReactAppTsx = (
  ...
  const containsReturn = returnRegex.test(codeSnippet);

  // Conditionally render "return(" in the returned output string
  const returnStatement = containsReturn ? "" : "return(";

  return `import React${fileExtension === "tsx" ? `, { FC }` : ""} from 'react';

    import { ${sortedICDSComponents.join(", ")} } from '@ukic/react';
    ${
      fileExtension === "tsx" && isPattern && getIcTypes.length > 0
        ? `import type { ${getIcTypes.join(
            ", "
...
export const reactCodeSnippetCSS = `@import "@ukic/fonts/dist/fonts.css";
@import "@ukic/react/dist/core/core.css";
@import "@ukic/react/dist/core/normalize.css";
`;

export const webComponentsCodeSnippetCSS = `@import url("https://unpkg.com/@ukic/fonts/dist/fonts.css");
