...
<p>
  ...
  rather than the implementation of the component.
</p>
<p>
  For example, a user would view a button by the label 'Add', so the library
...
<p>
  Shadow DOM Testing Library provides functions with 'Shadow' prefixed to the
  query type (for example,{" "}
  <code style={{ backgroundColor: "#EEEFF0" }}>getByShadowText()</code>).
</p>
