...
function findDeepInShadowDOM(root: Element | ShadowRoot, selector: string): Element | null {
  // Attempt to find the element directly within the current root
  let element = root.querySelector(selector);
  if (element) {
    return element;
  }

  // If not found, attempt to drill down into nested shadow roots
  ...
  for (let i = 0; i < shadowHosts.length; i++) {
    ...
    if (potentialShadowRoot) {
      ...
      if (element) {
        return element; // Found the element in a nested shadow root
      }
    }
  }

  return null; // Element not found in any shadow root
}
