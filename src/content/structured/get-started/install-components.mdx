---
path: "/get-started/development/install-components"

navPriority: 1

date: "2024-12-23"

title: "Install the components"

subTitle: "Install and use the UI Kit component library. Use React or web components to quickly build accessible interfaces."
---

import { IcAlert } from "@ukic/react";

## Installing the components

<p>
  The UI Kit components are available on{" "}
  <ic-link
    target="_blank"
    href="https://www.npmjs.com/"
    rel="noreferrer noopener nofollow"
  >
    npmjs.com
  </ic-link>
  , and can be installed from the command line.
</p>

```shell
# Web components, Angular, Vue, Svelte
npm install @ukic/web-components @ukic/fonts

# React
npm install @ukic/react @ukic/fonts
```

## Using the components

<IcAlert
  heading="Components automatically switch colours based on the user's system settings"
  variant="warning"
>
  <ic-typography slot="message">
    When the ic-theme component is not implemented, components will default to
    the user's system colour scheme. Please keep this in mind when styling your
    application. See the{" "}
    <a href="/components/theme#dark-mode-as-a-theme">theming guidance</a> for
    more information.
  </ic-typography>
</IcAlert>

To use the components in a particular framework, follow the framework instructions.

- [React](/get-started/development/install-components/react)
- [Angular](/get-started/development/install-components/angular)
- [Vue](/get-started/development/install-components/vue)
- [Svelte](/get-started/development/install-components/svelte)
- [Gatsby](/get-started/development/install-components/gatsby)
- [Next.js](/get-started/development/install-components/nextJS)

Web components can be used without the need for a framework. To use web components without a framework, follow the [web components instructions](/get-started/development/install-components/web-components).

<p>
  Custom Elements Everywhere has{" "}
  <ic-link
    target="_blank"
    href="https://custom-elements-everywhere.com"
    rel="noreferrer noopener nofollow"
  >
    more information on how web components work with other frameworks
  </ic-link>
  .
</p>

## Using third-party components

If you can't find a suitable component in this Design System, then you can use one from a third-party library. Use the library's theming and customisation options and follow our [styles](/styles) so that they look and feel as similar as possible.

You can replace the third-party components once they are released in the Design System.
