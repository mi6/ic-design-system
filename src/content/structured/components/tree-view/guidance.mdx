...
Tree view provides a navigation tree for use in applications with heirarchical organised pages.
...
<DoubleDoDontCaution>
  <DoDontCaution
    ...
    state="good"
    caption="Use tree view to navigate throughout heirarchical content."
  />
</DoubleDoDontCaution>
...
<DoubleDoDontCaution>
  <DoDontCaution
    ...
    state="good"
    caption="Use tree view to with icons and a title bar to provide context to your data."
  />
</DoubleDoDontCaution>
...
<DoubleDoDontCaution>
  <DoDontCaution
    ...
    state="bad"
    caption="Don't use tree view for data that would benefit from being displayed vertically."
  />
</DoubleDoDontCaution>

Avoid using tree view in a drawer/section with restricted width, as the nested layers for child tree items require width to expand into.

Avoid using multiple tree views on the same web page, consider using tabs to separate sections.

## Colour

Use the dark or light appearance to achieve colour contrast compliance when using tree view on different coloured backgrounds.

Icons can be used to provide a range of colours to your application.

## Sizing and placement

Set the tree view item’s vertical size dependent on the amount of tree view items being displayed within one tree view. Large items take up more height so are good to use when there are fewer navigation options. Small items allow for denser and more compact options so are best to use in situations with many nested layers of navigation items.

The width of the tree view is set by the screen size or the container that it is placed within. Tree view item text will be truncated if it overlaps the size of the container, and tooltips will highlight the original text string.

Place the tree view at either side of the page to ensure it can always be accessed independent of the screen content. Tree views should fill the whole height of the container that they sit within.
...
<DoDontCaution
  ...
  state="bad"
  caption="Place tree view within a wide enough container to properly indent nested data."
/>
