---
path: "/components/inputs/select/code"
...
export const SelectExampleSizes = () => (
...
<ComponentPreview snippets={multiSelect}>
  <IcSelect
    label="What are your favourite types of coffee?"
    value={["Ame", "Fil", "Moc"]}
    options={[
...
export const ReadonlyMulti = () => {
  const readonlyMultiSelectEl = useRef(null);
  useEffect(() => {
    readonlyMultiSelectEl.current.setAttribute("readonly", "true");
  }, []);
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <IcSelect
...
const inputHandler = (event) => {
  ...
  if (newValue !== selectedValue) {
    if (newValue && newValue.length > 1) {
      ...
      mockAPI(newValue).then((newResults) => { 
        setLoading(false);
