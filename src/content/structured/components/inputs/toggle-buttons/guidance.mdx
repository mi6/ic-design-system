---
path: "/components/inputs/toggle-button"
...
tabs:
  [
    { title: "Guidance", path: "/components/inputs/toggle-button" },
    { title: "Code", path: "/components/inputs/toggle-button/code" },
    {
      title: "Accessibility",
      path: "/components/inputs/toggle-button/accessibility",
    },
  ]
...
A toggle button is a type of button that shows a selection status and so is commonly used to turn a setting ‘on’ or ‘off’. It is similar to a [switch](/components/inputs/switch) but can be grouped into denser layouts, for example within a toolbar.
...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig1ToggleLight, Fig1ToggleDark]}
...
Use toggle buttons in dense layouts or sections of an app that don’t have a lot of space. Use other selection methods, such as [radio buttons](/components/inputs/radio) or [switches](/components/inputs/switch), when there is more space to include more text labelling.
...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig2ToggleLight, Fig2ToggleDark]}
...
Consider whether toggle buttons are the best selection method for your use case. [Radio buttons](/components/inputs/radio), [checkboxes](/components/inputs/checkbox) and [switches](/components/inputs/switch) provide alternative methods of proving a selection.
...
Avoid using toggle buttons to control the state of another content section in your app. Instead, use [tabs](/components/navigation/tabs) for switching views.
...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig3ToggleLight, Fig3ToggleDark]}
...
Use the `theme` and `monochrome` props when placing toggle buttons on different coloured backgrounds. This makes sure that a good colour contrast can be achieved in adherence to the accessibility guidelines.

Don’t mix different coloured buttons in the same toggle button group, always use the same colour for all toggle buttons within a group.
...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig4ToggleLight, Fig4ToggleDark]}
    imageAlt="Two versions of a text styling setting that uses toggle button groups. One has a dark background with light theme toggle buttons, and the other has a light coloured background with dark theme toggle button groups."
    state="good"
    caption="Use the appearance options to achieve good contrast on different coloured backgrounds."
  />
  <DoDontCaution
    imageSrc={[Fig5ToggleLight, Fig5ToggleDark]}
    imageAlt="A text styling setting that uses toggle button groups. Within the toggle button groups different toggle buttons use different monochrome prop values."
    state="bad"
    caption="Don’t mix theme and monochrome props in the same toggle button group."
  />
</DoubleDoDontCaution>
...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig6ToggleLight, Fig6ToggleDark]}
...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig7ToggleLight, Fig7ToggleDark]}
...
Aim to only include two to six toggle buttons within a toggle button group. The larger the number of buttons the less horizontal space is available to display the labels. If more options are required, consider using a [popover menu](/components/navigation/popover-menu) instead.
