...
It is important that the selected state can be clearly identified from the unselected state. To conform with the WCAG criteria <ic-link target="_blank" href="https://www.w3.org/TR/WCAG22/#use-of-color" rel="noreferrer noopener nofollow">1.4.1 Use of color</ic-link>, the toggle button does not solely rely on colour to indicate the selection state, but instead makes use of an additional shape added to the interior of the button to clearly distinguish it from its unselected state.
