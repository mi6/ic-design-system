---
path: "/components/inputs/search-bar"
...
By default, the search bar uses `searchMode="navigation"`, which is optimised for selecting from a predefined list of options. In this mode, if there are no matching options, a "No results found" message is shown and the submit button is disabled.
...
Set `searchMode="query"` to allow users to submit any search term, even if it does not match a predefined option. This is useful for free-form searching, such as when querying a database or API, as the search bar will always allow submission regardless of the available options.
...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig1SearchLight, Fig1SearchDark]}
...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig3SearchLight, Fig3SearchDark]}
  ...
  />
  <DoDontCaution
    imageSrc={[Fig4SearchLight, Fig4SearchDark]}
...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig5SearchLight, Fig6SearchDark]}
  ...
  />
  <DoDontCaution
    imageSrc={[Fig6SearchLight, Fig6SearchDark]}
