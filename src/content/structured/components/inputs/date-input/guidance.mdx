---
path: "/components/inputs/date-input"
...
Use a date input to select a past, present or future date. For example, use a date input when asking for a person’s birthday as it may be quicker than navigating through a [date picker](/components/inputs/date-picker).

<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={[Fig1DateInputLight, Fig1DateInputDark]}
    imageAlt="A date input being used to select a birthday"
    caption="Use a date input to collect distant or known dates."
    state="good"
  />
</DoubleDoDontCaution>
