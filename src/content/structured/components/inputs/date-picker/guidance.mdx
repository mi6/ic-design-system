...
Dates that are disabled by the date picker's restrictions cannot be selected from the calendar, so selecting a date in the picker will not trigger validation for those restrictions.

People can still type a date directly into the date input, including a date that is outside the allowed range. See [date input validation](/components/inputs/date-input#validation) for guidance on displaying and writing validation errors for manually entered dates.
