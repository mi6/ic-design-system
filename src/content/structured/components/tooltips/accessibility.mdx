...
Tooltips have an internal `silent` property, which applies `aria-hidden` to the tooltip when the tooltip information would have been duplicated e.g. when an icon button has a `title` or `aria-label` set.
