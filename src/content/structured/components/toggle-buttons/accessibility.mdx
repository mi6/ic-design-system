...
For a single select group, the whole group is focussed upon using the ‘tab’ key and the options are navigated using the ‘arrow’ keys, such as with a [radio button group](../components/radios). For a multi-select group each toggle button in the group is individually focusable with the ‘tab’ key.
