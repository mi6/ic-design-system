...
Avoid using pagination bar with linear content such as articles or blogs. Scrolling down the page makes it easier to consume this content.

Avoid using pagination bar when the size of the content is unknown or if it frequently changes. This causes the number of pages to change frequently and means it is hard to create a ‘last page’ option.
...
Try to place pagination bar at the bottom of the page it controls, as this encourages the user to read through the page before moving to the next. However, placing another method of pagination at the top of the page is still a valid option.

<DoDontCaution
  imageSrc={[Fig3PaginationBarLight, Fig3PaginationBarDark]}
  imageAlt="Pagination bar shown below a container."
  state="good"
  caption="Try to place pagination bar at the bottom of the container it controls."
/>
...
<DoDontCaution
  ...
  imageAlt="Pagination bar below a container with Complex pagination above the container for additional control."
  state="good"
