...
<DoubleDoDontCaution>
  <DoDontCaution
    imageSrc={buttonFig5}
    imageAlt="A form with correctly sized components. A normal sized input component is paired with normal sized button, and a dense sized input component is paired with a dense sized button."
    state="good"
    caption="Use the correct buttons sizes to match other UI component sizes."
  />
  <DoDontCaution
    imageSrc={buttonFig6}
    imageAlt="A form with incorrectly sized components. A normal sized input component is paired with dense sized button, and a dense sized input component is paired with a normal sized button."
    state="bad"
