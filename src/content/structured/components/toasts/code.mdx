...
export const ToastWithoutRefSnippet = [
  {
    technology: "React",
    snippets: {
      short: `<IcButton onClick={handleClick}>
  Display toast on load without useRef
</IcButton>
<IcToastRegion>
  <IcToast
  ...
  />
    ...
    const toastRegion = document.querySelector(
      "ic-toast-region",
    ) as HTMLIcToastRegionElement;
    const toastElement = document.getElementById("toast") as HTMLIcToastElement;
...
<ComponentPreview snippets={ToastWithoutRefSnippet}>
  <ToastWithoutRefs />
</ComponentPreview>
