...
- Fixes pagination issue where custom label not applied.
- Handle small data list with no heading.
- Use size prop instead of non-existing small prop.
- Warn when backBreadcrumbOnly is used with only one item.
- Add anchor slot to popover menu.
- Add props to prevent closing of popover when options pressed.
- Only emit icTimeChange once milliseconds field is completed.
- Update styling of text and clear button for disabled date picker.
- Component docs for closeMenuOnSelect.
