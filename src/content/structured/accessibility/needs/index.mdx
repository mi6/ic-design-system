...
Common accessible designs include:

### Large font

Writing in a large font size will put less writing on a page and make the words more readable. Our eyes are most often drawn to the larger text on a page. Less words on a page will feel like less effort to read.

### Tabular formatted buttons

Allowing people to tab between each button instead of using the mouse. Everything must be be accessible with this method for people who don’t like using the mouse to click into sections.

### Descriptive buttons

Ensure that buttons describe their feature. Having a button or link that says ‘Click here’ isn’t beneficial if ‘here’ isn’t specified. Using descriptions that say exactly what the button will do is more accessible.
...
Before confirming a design, reach out to the wider community (both with and without accessibility needs) to test the design and find out which components are beneficial and which aren’t.
